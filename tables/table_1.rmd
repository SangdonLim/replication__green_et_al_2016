---
title: "Replication of Table 1 of Green et al (2016)"
...
# Table 1 Part 1
dd <- subset(d, m_type == "a")
oo <- aggregate(dd, by = list(dd$t_type, dd$n_obs), mean)
oo <- oo[, c(
  "Group.1", "w_lambda",
  "T_ru_hit", "T_ru_under", "T_ru_over",
  "R_ru_hit", "R_ru_under", "R_ru_over"
)]
table_data_component <- cbind(oo[1:2, ], oo[3:4, ])
table_data_component <- table_data_component[
  order(table_data_component[["Group.1"]], decreasing = TRUE),
]
table_data <- table_data_component

# Table 1 Part 2
dd <- subset(d, m_type == "b")
...
```{r, echo = FALSE}
table_data_original <- read.csv(file.path(root, "tables/table_1_original.csv"))
for (i in 3:14) {
